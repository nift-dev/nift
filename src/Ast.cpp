...
bool evaluate(const Expr& e,Context& c,json::Document& out,std::string& error){
 ...
 if(e.kind==Kind::Member){json::Document b;if(!evaluate(*e.left,c,b,error))return false;if(!b.is_object()||!b.has(e.name)){error="value has no member: "+e.name;return false;}out=b[e.name];return true;}
 if(e.kind==Kind::Index){json::Document b,i;if(!evaluate(*e.left,c,b,error)||!evaluate(*e.right,c,i,error))return false;if(b.is_array()&&i.is_number()&&i.num>=0&&std::trunc(i.num)==i.num&&(std::size_t)i.num<b.array.size()){out=b.array[(std::size_t)i.num];return true;}if(b.is_object()&&i.is_string()&&b.has(i.string)){out=b[i.string];return true;}error="invalid index";return false;}
