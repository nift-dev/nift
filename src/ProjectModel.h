...
inline json::Document compute_tracked_metadata(const std::filesystem::path& root, const Config& config, const TrackedInfo& info, const content_model::Model& model, std::string* body_out = nullptr) {
    ...
    if (metadata.has("type")) {
        if (!metadata["type"].is_string()) metadata["_error"] = json::Document("front matter type must be a string");
        else if (!type.empty() && metadata["type"].string != type) metadata["_error"] = json::Document("tracked type conflicts with front matter type");
        else type = metadata["type"].string;
    }
    if (!type.empty()) metadata["type"] = json::Document(type);
    if (!type.empty() && !metadata.has("_error")) {
        auto si = model.schemas.find(type);
        if (si == model.schemas.end()) { if (!config.schema_files.empty()) metadata["_error"] = json::Document("unknown content schema/type: " + type); }
        else { std::string ve; if (!content_model::validate(metadata, si->second, model, ve)) metadata["_error"] = json::Document(ve); }
    }
    if (body_out) *body_out = body;
    return metadata;
}

inline std::shared_ptr<const json::Document> make_project_value(const std::filesystem::path& root, const Config& config, const std::vector<TrackedInfo>& tracked, const content_model::Model* shared_model = nullptr) {
    ...
    const content_model::Model model = shared_model ? *shared_model : content_model::load(root, config.schema_files, config.taxonomy_files);
    ...
    for(const auto& info:tracked){
        ...
        std::string body;
        json::Document metadata = compute_tracked_metadata(root, config, info, model, &body);
        if(info.frontmatter)f["frontmatter_source"]=json::Document(*info.frontmatter);
        f["metadata"]=metadata; p["files"].push_back(f);
        std::string type; if(info.type)type=*info.type; if(metadata.has("type")&&metadata["type"].is_string())type=metadata["type"].string;
        if(!type.empty()&&!metadata.has("_error")){
            ...
            if(!p["content"].has(type))p["content"][type]=json::Document::make_array();
            p["content"][type].push_back(c);p["content"]["all"].push_back(c);
            if(const auto si2=model.schemas.find(type);si2!=model.schemas.end()){const auto& sch=si2->second;for(const auto&fld:sch.fields)if(fld.type=="taxonomy"&&metadata.has(fld.name)){auto add=[&](const json::Document&v){if(v.is_string())tax_content[fld.taxonomy][v.string].push_back(c);};if(fld.array&&metadata[fld.name].is_array())for(const auto&v:metadata[fld.name].array)add(v);else add(metadata[fld.name]);}}
        }
    }
    for(auto&tk:tax_content){if(!p["taxonomies"].has(tk.first))continue;auto& tax=p["taxonomies"][tk.first];std::set<std::string> slugs;for(auto&termkv:tk.second){json::Document term=json::Document::make_object();term["name"]=json::Document(termkv.first);std::string slug;for(char ch:termkv.first){unsigned char c=(unsigned char)ch;if(c>=128)slug+=ch;else if(std::isalnum(c))slug+=(char)std::tolower(c);else if((ch==' '||ch=='_'||ch=='/'||ch=='-')&&!slug.empty()&&slug.back()!='-')slug+='-';}while(!slug.empty()&&slug.back()=='-')slug.pop_back();if(slug.empty()||!slugs.insert(slug).second){if(!p.has("errors"))p["errors"]=json::Document::make_array();p["errors"].push_back(json::Document("taxonomy "+tk.first+": duplicate or empty slug for term '"+termkv.first+"'"));continue;}term["slug"]=json::Document(slug);term["content"]=json::Document::make_array();for(auto&c:termkv.second)term["content"].push_back(c);bool hierarchical=false;if(auto ti=model.taxonomies.find(tk.first);ti!=model.taxonomies.end())hierarchical=ti->second.hierarchical;if(hierarchical&&termkv.first.find('/')!=std::string::npos){auto pos=termkv.first.rfind('/');term["parent"]=json::Document(termkv.first.substr(0,pos));}tax.push_back(term);} }
