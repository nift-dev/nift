...
static int package_add_cli(int argc,char**argv){
    ...
    if(local){fs::path src=fs::absolute(source).lexically_normal();if(!package_validate_manifest(src,name,error)){console::error(error);return 1;}if(conflict_guard(name))return 1;dest=package_root()/name;fs::remove_all(dest,ec);fs::create_directory_symlink(src,dest,ec);if(ec){console::error("cannot link local package: "+ec.message());return 1;}commit="local";}
