...
Nift has completed its planned Checkpoints 0–10 deliberate hardening campaign, followed by the v4.2 and v4.3 language/content campaigns and the v4.4 execution/shell/package campaign. The current development tree is `Nift v4.4.0`, following the public v4.1.0/v4.2.0/v4.3.0 releases. The v4.4 surface adds native process execution with structured `run()`/`cmd()` results, the Bash-like interactive `nift sh` shell with completion, project/build automation APIs, project and per-file build hooks, opt-in restrictions (`--no-process`, `--fs-root`), a Git-native package system with the first `sqlite` package, variadics/spread, wildcards and rendered block lambdas. Existing regression, sanitizer, filesystem, parser, incremental, cross-platform, performance and packaging gates remain maintained; new hardening work should be driven by concrete findings or newly justified guarantees rather than arbitrary checkpoint numbers.
