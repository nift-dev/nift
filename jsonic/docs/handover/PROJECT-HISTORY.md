...
## 2026-09-13 — Opt-in configuration parsing

Added independent opt-ins for comments and trailing commas without weakening
the strict default. The same additive API exposes structured byte/line/column
diagnostics, caller-selected nesting limits and optional duplicate-key
rejection. Duplicate preservation remains the default, and no per-node source
metadata was added. Standalone, adversarial and external API coverage establish
the strict/extended boundary and option composition.
