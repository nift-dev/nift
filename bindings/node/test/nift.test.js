...
(async () => {
  await test("abi: values/bindings render", async () => {
    ...
    e.setBool("b", true);
    e.setJSON("v", '{"x":1,"y":"z"}');
    const r = await e.render("$[s]|$[i]|$[n]|$[b]|$[v.x]|$[v.y]", "<main>@content</main>");
  ...
  await test("context bindings + context-over-engine precedence", async () => {
    ...
    const r = await e.render("$[site]", "<main>@content</main>", c);
  ...
  await test("malformed JSON content -> error_prefix family", async () => {
    ...
    const e = Engine.new();
    e.setRoot(root);
    const r = await e.render('@json("content/bad.json", d)$[d.x]@content', "<main>@content</main>");
  ...
  await test("loader Found / NotFound / Error(diagnostic)", async () => {
    ...
    let r = await e.render('@input("p.html")', "<main>@content</main>");
    ...
    r = await e2.render('@input("missing.html")', "<main>@content</main>");
    assert.strictEqual(r.ok, false);
    ...
    r = await e3.render('@input("p.html")', "<main>@content</main>");
    assert.strictEqual(r.ok, false);
  ...
  await test("environment Found / NotFound / Error(diagnostic)", async () => {
    ...
    let r = await e.render("@getenv(GREETING)", "<main>@content</main>");
    ...
    r = await e2.render("@getenv(X)", "<main>@content</main>");
    assert.strictEqual(r.ok, false);
  ...
  await test("page/pagination render with project engine", async () => {
    ...
    const r = await e.renderPage("blog");
    assert.strictEqual(r.ok, true);
  ...
  await test("partial render", async () => {
    const e = Engine.new();
    const r = await e.renderPartial("<p>frag</p>");
  ...
  await test("concurrent renders with callbacks", async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 64 }, (_, i) =>
        e.render('@input("p.html")', "<main>$[n]</main>@content", (() => {
          const c = new Context();
          c.setInt("n", i);
          return c;
        })())
  ...
  await test("callbacks survive GC", async () => {
    ...
    if (global.gc) global.gc();
    const r = await e.render('@input("p.html")', "<main>@content</main>");
  ...
  await test("repeated engine create/dispose (lifetime)", async () => {
    for (let i = 0; i < 200; i++) {
      ...
      const r = await e.render("$[s]", "<main>@content</main>");
  ...
  await test("GC pressure: many engines/contexts without leaks", async () => {
    for (let i = 0; i < 100; i++) {
      ...
      const r = await e.render("$[s]", "<main>@content</main>", c);
      assert.strictEqual(r.ok, true);
      c.close();
      e.close();
    }
  ...
  await test("exception containment: throwing in loader", async () => {
    ...
    const r = await e.render('@input("x.html")', "<main>@content</main>");
    assert.strictEqual(r.ok, false);
  ...
  await test("long-lived engine across many renders", async () => {
    ...
    for (let i = 0; i < 500; i++) {
      const r = await e.render("$[site]", "<main>@content</main>");
