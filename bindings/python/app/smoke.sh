#!/usr/bin/env bash
# Python WSGI dogfood acceptance smoke test.
set -euo pipefail
cd "$(dirname "$0")"
PYTHON_DIR="$(cd .. && pwd)"

PORT="${NIFT_DOGFOOD_PORT:-5201}"
BASE="http://127.0.0.1:$PORT"
LOG="$(mktemp "${TMPDIR:-/tmp}/nift-py-dogfood.XXXXXX.log")"
trap 'kill -- -"$APP_PID" 2>/dev/null || true; rm -f "$LOG"' EXIT

echo "building python binding"
bash "$PYTHON_DIR/build.sh" >/dev/null

echo "starting python dogfood on :$PORT"
cd "$PYTHON_DIR"
PORT="$PORT" setsid python3 -m app.app >"$LOG" 2>&1 &
APP_PID=$!
...
printf '%s' "$body" | grep -Fq "Nift Python Dogfood" || fail "/ missing engine default title"
...
printf '%s' "$concurrency" | grep -Fq '"failures": 0' || fail "/concurrency evidence missing: $concurrency"
