...
CASES = [
    ("truncated-json", "@json('data/site.json',site)\n@content\n", '{"a:', {}, False, None),
    ("invalid-json", "@json('data/site.json',site)\n@content\n", 'not json', {}, False, None),
    ("deep-nesting", "@json('data/site.json',site)\n@if(site.ok){ok}\n@content\n",
     '{"ok":' + ('[' * 200) + (']' * 200) + '}', {}, True, None),
    ("deep-nesting-valid", "@json('data/site.json',site)\n@if(site.ok){ok}\n@content\n",
     '{"ok":true}', {}, True, ['ok']),
    ("type-mismatch-array", "@json('data/site.json',site)\n@for(x:site.items){<i>$[x]</i>}\n@content\n",
     '{"items":42}', {}, False, None),
    ("missing-key", "@json('data/site.json',site)\n@if(site.nope){x}\n@content\n", '{}', {}, False, None),
    ("null-value", "@json('data/site.json',site)\n$[site.x]\n@content\n", '{"x":null}', {}, True, None),
    ("string-as-number", "@json('data/site.json',site)\n@if(site.n >= 2){big}\n@content\n",
     '{"n":"5"}', {}, False, None),
    ("number-as-string", "@json('data/site.json',site)\n$[site.s]\n@content\n",
     '{"s":123}', {}, True, ['123']),
    ("unicode", "@json('data/site.json',site)\n$[site.s]\n@content\n",
     '{"s":"\\u00e9\\ud83d\\ude00"}', {}, True, None),
    ("unicode-valid", "@json('data/site.json',site)\n$[site.s]\n@content\n",
     '{"s":"\\u00e9"}', {}, True, ['\u00e9']),
    ...
    ("for-over-object", "@json('data/site.json',site)\n@for(k:site.obj){<i>$[k]</i>}\n@content\n",
