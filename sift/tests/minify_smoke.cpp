...
int main() {
    ...
    expect(sift::json(R"( { "a" : 1, "s" : "a b", "x" : [ true, null ] } )", out, err), err);
    eq(out, R"({"a":1,"s":"a b","x":[true,null]})", "json compact");
    expect(!sift::json(R"({"a":})", out, err), "invalid JSON accepted");
    expect(err.find("invalid JSON") != std::string::npos, "invalid JSON error missing");

    expect(sift::css("/*x*/ body  { color : red ; margin : 0  10px ; }", out, err), err);
    eq(out, "body{color:red;margin:0 10px;}", "css basic");
    expect(sift::css("/*!license*/ .x { content: \"a  b\"; }", out, err), err);
    ...
    expect(sift::html("  <div   class=\"a  b\">  hello   world <!-- gone --> <span> x </span> </div>  ", out, err), err);
    eq(out, "<div class=\"a  b\"> hello world <span> x </span> </div>", "html basic");
    expect(sift::html("<pre>  a\n    b </pre><script> const x = ` a  b `;\n</script>", out, err), err);
    ...
    expect(sift::css("@media2 (width > 10px) { .x { color: red; } }", out, err), err);
    expect(out.find("@media2") != std::string::npos, "future CSS at-rule rejected or damaged");
    expect(sift::css(":root { --gap: calc(100% - 2rem); --blob: url(\"data:image/svg+xml,%3Csvg%20viewBox='0 0 1 1'%3E%3C/svg%3E\"); }", out, err), err);
    ...
    expect(sift::css("@container sidebar (width > 30rem) { .card { container-type: inline-size; } }", out, err), err);
    ...
    expect(sift::css("@layer reset, base, theme; @layer theme { .x { color: color(display-p3 1 0 0 / .5); } }", out, err), err);
    expect(out.find("@layer") != std::string::npos && out.find("display-p3") != std::string::npos,
           "CSS layer/color syntax damaged");
    expect(sift::css("@supports selector(:has(*)) { .a:has(> .b) { width: clamp(1rem, 2vw + 1rem, 3rem); } }", out, err), err);
    ...
    expect(sift::css(".a { & > .b { --tokens: {a:b}; margin-inline: 1cqi; } }", out, err), err);
    ...
    expect(sift::html("<span>a</span> <span>b</span><div> c </div>", out, err), err);
    expect(out.find("</span> <span>") != std::string::npos, "HTML inline inter-element space erased");
    expect(sift::html("<script><!-- not an html comment --></script><style>/* raw */ .x { a: b; }</style>", out, err), err);
    ...
    expect(!sift::html("<div class=\"x\"", out, err), "malformed HTML tag accepted");
    expect(err.find("unterminated HTML tag") != std::string::npos, "malformed HTML error missing");
    expect(sift::html("<p>héllo 😀 世界</p>", out, err), err);
    expect(out.find("héllo 😀 世界") != std::string::npos, "HTML Unicode damaged");
    expect(sift::html("<!doctype html><template><span>A</span> <span>B</span></template>", out, err), err);
    ...
    expect(sift::javascript("while (condition) ;\nnext();", out, err), err);
    expect(out.find("while(condition);") != std::string::npos, "empty while-loop statement semicolon stripped");
    expect(sift::javascript("const a=/[/]/g; const b=/a\\/\\/b/; const c=/[/*]/;", out, err), err);
    ...
    expect(sift::javascript("const t=`hello ${name} ${`nested ${value}`}`;\n", out, err), err);
    expect(out.find("`hello ${name} ${`nested ${value}`}`") != std::string::npos, "nested template literal damaged");
    expect(sift::javascript("const el = <div className=\"x\">hello world {name}</div>;\n", out, err), err);
    expect(out.find("hello world") != std::string::npos, "JSX text whitespace damaged");
    expect(sift::javascript("type User = { name: string }; const x: User = { name: 'A' };\n", out, err), err);
    ...
    expect(sift::javascript("if (ok) /https?:\\/\\//.test(url);\n", out, err), err);
    expect(out.find("/https?:\\/\\//") != std::string::npos, "regex after control-flow condition damaged");
    expect(sift::javascript("while (ok) /a\\/\\/b/.test(s);\n", out, err), err);
    ...
    expect(sift::javascript("const z = value / /a\\/\\/b/.test(s);\n", out, err), err);
    expect(out.find("/a\\/\\/b/") != std::string::npos, "division/regex boundary damaged");

    expect(sift::javascript("const π = 3.14; const 世界 = 'ok';\n", out, err), err);
    expect(out.find("π") != std::string::npos && out.find("世界") != std::string::npos, "JS Unicode damaged");
    expect(!sift::javascript("const r = /unterminated\nx();", out, err), "unterminated JS regex accepted");

    // Idempotence: a second pass must be byte-for-byte stable for each format.
    std::string once, twice;
    expect(sift::json(" { \"x\" : [1, 2], \"s\":\"a b\" } ", once, err), err);
    expect(sift::json(once, twice, err), err); eq(twice, once, "json idempotence");
    expect(sift::css("/*!x*/ .a { width: calc(100% - 2rem); color: red; }", once, err), err);
    expect(sift::css(once, twice, err), err); eq(twice, once, "css idempotence");
    expect(sift::html("<div> a <span>b</span> c </div>", once, err), err);
    expect(sift::html(once, twice, err), err); eq(twice, once, "html idempotence");
    expect(sift::javascript("while(x); // keep semantics\nconst r=/a\\/b/;\n", once, err), err);
    expect(sift::javascript(once, twice, err), err); eq(twice, once, "js idempotence");
    expect(sift::jsx("const x=<span>https://example.com a  b</span>;\n", once, err), err);
    expect(sift::jsx(once, twice, err), err); eq(twice, once, "jsx idempotence");


    // Explicit JSX entry point is independently addressable by extension.
    expect(sift::jsx("const x = <><span>a b</span><span>{value}</span></>;\n", out, err), err);
    expect(out.find("</>") != std::string::npos, "JSX fragment damaged");
    expect(sift::jsx("const x=<p>https://example.com/a // literal text</p>;\n", out, err), err);
    ...
    expect(!sift::javascript("const x = 'unterminated", out, err), "unterminated JS string accepted");
    expect(!sift::javascript("const x = `unterminated", out, err), "unterminated JS template accepted");
    expect(!sift::css("a{/* unterminated", out, err), "unterminated CSS comment accepted");
    ...
    expect(sift::javascript("const x = `head ${`inner ${v}`} raw // still text`;\n", out, err), err);
    ...
    expect(sift::css(R"CSS(.x { width: calc(100% - 2rem); --custom: 1  2; background:url("data:image/svg+xml,<svg><!--x--></svg>"); })CSS", out, err), err);
    expect(out.find("data:image/svg+xml") != std::string::npos, "unquoted CSS data URL damaged");


    // JavaScript lexical/ASI edge cases.
    expect(sift::javascript("try{}catch{} /https?:\\/\\//.test(s);", out, err), err);
    expect(out.find("/https?:\\/\\//") != std::string::npos,
           "regex after catch-without-binding block damaged");
    expect(sift::javascript("if(false){} /https?:\\/\\//.test(s);", out, err), err);
    expect(out.find("/https?:\\/\\//") != std::string::npos,
           "regex statement after block damaged");
    expect(sift::javascript("const x=function(){} / 2;", out, err), err);
    expect(out.find("}/2") != std::string::npos || out.find("} /2") != std::string::npos,
           "function-expression division mistaken for regex");
    expect(sift::javascript("const x=async function(){} / 2;", out, err), err);
    expect(out.find("}/2") != std::string::npos || out.find("} /2") != std::string::npos,
           "async function-expression division mistaken for regex");
    expect(sift::javascript("const x=class {static valueOf(){return 12}} / 2;", out, err), err);
    expect(out.find("}/2") != std::string::npos || out.find("} /2") != std::string::npos,
           "class-expression division mistaken for regex");
    expect(sift::javascript("const x=class X {static valueOf(){return 12}} / 2;", out, err), err);
    expect(out.find("}/2") != std::string::npos || out.find("} /2") != std::string::npos,
           "named class-expression division mistaken for regex");
    expect(sift::javascript("class C{} /https?:\\/\\//.test(s);", out, err), err);
    expect(out.find("/https?:\\/\\//") != std::string::npos,
           "regex after class declaration damaged");
    expect(sift::javascript("class C{} /[/*}]/.test(s);", out, err), err);
    expect(out.find("/[/*}]/") != std::string::npos,
           "regex character class after class declaration mistaken for comment");
    expect(sift::javascript("label:{} /https?:\\/\\//.test(s);", out, err), err);
    expect(out.find("/https?:\\/\\//") != std::string::npos,
           "regex after labelled block damaged");
    expect(sift::javascript("const x=true?1:{valueOf(){return 12}} / 2;", out, err), err);
    expect(out.find("}/2") != std::string::npos || out.find("} /2") != std::string::npos,
           "ternary object division mistaken for labelled block regex");
    expect(sift::javascript("function f(){} /a/.test(s);", out, err), err);
    expect(out.find("/a/.test") != std::string::npos, "regex after function declaration damaged");
    expect(sift::javascript("async function f(){for await(const x of xs) /a/.test(x);}", out, err), err);
    expect(out.find("/a/.test") != std::string::npos, "regex after for-await control parenthesis damaged");
    expect(sift::javascript("const n={valueOf(){return 12}} / 2;", out, err), err);
    expect(out.find("}/2") != std::string::npos || out.find("} /2") != std::string::npos ||
           out.find("}/ 2") != std::string::npos,
           "object-literal division mistaken for regex after brace");
    expect(sift::javascript("const n=({valueOf(){return 12}}) / 2 / d;", out, err), err);
    ...
    expect(sift::css("@media2 (width > 10px) { .x { future-prop: future-fn(1, 2); } }", out, err), err);
    ...
    expect(sift::css("@supports selector(:has(*)) { @container card (width > 20rem) { .x { color: oklch(60% .2 20); } } }", out, err), err);
    ...
    expect(!sift::html("<div", out, err), "unterminated HTML tag accepted");
    expect(!sift::css("a{/*", out, err), "unterminated CSS comment accepted");
    expect(!sift::javascript("/*", out, err), "unterminated JS comment accepted");

    // Idempotence: a second minification pass must be byte-identical.
    auto idem = [&](sift::Format fmt, const std::string& src, const char* label) {
        std::string a,b,e;
        expect(sift::run(fmt,src,a,e), e);
        expect(sift::run(fmt,a,b,e), e);
        if (a != b) {
            std::cerr << label << " is not idempotent\\nfirst: [" << a << "]\\nsecond:[" << b << "]\\n";
            std::exit(1);
        }
    };
    idem(sift::Format::Json, R"({"x":[1, 2],"s":"a b"})", "JSON");
    idem(sift::Format::Html, "<div>  A <span>B</span> C </div>", "HTML");
    idem(sift::Format::Css, "/*!x*/ .a { width: calc(100% - 1rem); }", "CSS");
    idem(sift::Format::JavaScript, "const r=/a\\\\/b/g; //x\nwhile(ok);\n", "JavaScript");


    {
        const std::string jsx_src = "const  el = <div>https://example.com/{ name + 1 }</div>;";
        expect(sift::jsx(jsx_src, out, err), err);
    ...
    idem_fn(sift::jsx, "const x=<div> hello  world </div>;", "JSX");


    // JSX: preserve text/markup spelling, but minify embedded JS expressions and
    // ordinary JS surrounding the JSX region.
    expect(sift::jsx("const  el = <div className=\"x\"> hello  world {  value +  1  } </div> ;", out, err), err);
    ...
    expect(!sift::xml("<root", out, err), "unterminated XML tag accepted");
    expect(!sift::xml("<root><!--", out, err), "unterminated XML comment accepted");

    // SVG shares XML's conservative text rules: visible <text>/<tspan> content
    // must not be collapsed merely to save bytes.
    expect(sift::xml("<p><b>A</b> <i>B</i></p>", out, err), err);
    expect(out.find("</b> <i>") != std::string::npos, "XML mixed-content whitespace removed");
    expect(sift::svg("<text><tspan>A</tspan> <tspan>B</tspan></text>", out, err), err);
    expect(out.find("</tspan> <tspan>") != std::string::npos, "SVG text-node whitespace removed");
    expect(sift::xml("<root xmlns:x=\"urn:x\"><x:item a=\"1 &amp; 2\">A &lt; B</x:item></root>", out, err), err);
    expect(out.find("xmlns:x=\"urn:x\"") != std::string::npos && out.find("A &lt; B") != std::string::npos,
           "XML namespace/entity content damaged");
    expect(sift::xml("<root><![CDATA[ x ]]> text <![CDATA[ y < z ]]></root>", out, err), err);
    expect(out.find("<![CDATA[ x ]]> text <![CDATA[ y < z ]]>") != std::string::npos,
           "XML adjacent CDATA/mixed text damaged");
    expect(sift::svg("<svg viewBox=\"0 0 10 10\"><path d=\"M 0 0 L 10 10 Z\"/><text xml:space=\"preserve\"> A  B </text></svg>", out, err), err);
    ...
    idem(sift::Format::Xml, "<root>\\n <a> text  here </a>\\n</root>", "XML");
    idem(sift::Format::Svg, "<svg>\\n<text>a  b</text>\\n</svg>", "SVG");
    idem(sift::Format::Jsx, "const x = <div>{ value + 1 }</div>;", "JSX");

    sift::Format f;
    expect(sift::format_for_extension(".html", f) && f == sift::Format::Html, "html extension");
    expect(sift::format_for_extension("MJS", f) && f == sift::Format::JavaScript, "mjs extension");
    expect(sift::format_for_extension(".jsx", f) && f == sift::Format::Jsx, "jsx extension");
    expect(sift::format_for_extension(".xml", f) && f == sift::Format::Xml, "xml extension");
    expect(sift::format_for_extension(".svg", f) && f == sift::Format::Svg, "svg extension");
    expect(!sift::format_for_extension(".ts", f), "TypeScript source extension unexpectedly supported");
    expect(!sift::format_for_extension(".tsx", f), "TSX source extension unexpectedly supported");
